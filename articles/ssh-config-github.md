...
今回は同一接続先に対してアカウントを切り替えたいので、`Host` にはアカウント識別可能な値を、`HostName` に接続先である `github.com` を明示的に指定してアカウント毎に設定をわけました。

次に `git` のリモートリポジトリへの接続情報を変更します。

---

`~/.ssh/config` に `User` を指定している場合は該当 `Host` へ接続時に自動的にユーザが設定されるため、接続 URL からユーザ名である `git` を取り除いても問題ありません。
